...
const COPY = {
  ru: {
    ...
    articles: [
      ...
      ['Передача третьим лицам', 'Для работы сервиса часть данных обрабатывается сторонними сервисами: хостинг и хранение медиафайлов (DigitalOcean), база данных (Railway), защита форм от спама (Google reCAPTCHA), уведомления (Telegram и электронная почта), а также — только если вы сами подключили аккаунт — статистика просмотров (TikTok, Instagram). Кроме того, страницы сайта подгружают шрифты с серверов Google (fonts.googleapis.com, fonts.gstatic.com) и графику эмодзи с CDN jsDelivr; этим сервисам при загрузке страницы передаётся ваш IP-адрес и стандартные данные браузера — это технически необходимо для отображения страницы и не используется нами для идентификации. Мы не продаём ваши данные и не передаём их для рекламных рассылок третьих лиц.'],
      ['Хранение за рубежом', 'Данные размещаются на серверах провайдеров в дата-центрах Евросоюза: хостинг и медиафайлы — во Франкфурте (Германия, DigitalOcean), база данных — в Амстердаме (Нидерланды, Railway). Отправляя данные через формы, вы соглашаетесь на их трансграничную передачу и хранение. Передача защищена шифрованием TLS.'],
      ['Ваши права', null],
  ...
  en: {
    ...
    articles: [
      ...
      ['Third parties', 'To run the service, some data is processed by third-party services: hosting and file storage (DigitalOcean), database (Railway), form spam protection (Google reCAPTCHA), notifications (Telegram and email), and — only if you connected an account yourself — view statistics (TikTok, Instagram). In addition, site pages load fonts from Google’s servers (fonts.googleapis.com, fonts.gstatic.com) and emoji graphics from the jsDelivr CDN; loading a page sends your IP address and standard browser data to those services — this is technically required to render the page and is not used by us for identification. We do not sell your data or share it for third-party marketing.'],
      ['Storage abroad', 'Data is hosted on providers’ servers in EU data centers: hosting and files in Frankfurt, Germany (DigitalOcean), and the database in Amsterdam, Netherlands (Railway). By submitting data through the forms you consent to its cross-border transfer and storage. Transfer is protected with TLS.'],
      ['Your rights', null],
